...
.formatWeights <- function(file = "#weights") {

}

#' @title
#' Function to generate prevalence
#' @param weights census derived weights
...
generateCohortPrevalence <- function(executionSettings,
  ...
  if (!is.null(weights)) {
    weightTableName <- "#acs_weights"

    weightSql <- ClinicalCharacteristics:::.insertTableSql(
      executionSettings = executionSettings,
      tableName = weightTableName,
      data = weights
    )
  } else {
    weightTableName <- NULL
  }
